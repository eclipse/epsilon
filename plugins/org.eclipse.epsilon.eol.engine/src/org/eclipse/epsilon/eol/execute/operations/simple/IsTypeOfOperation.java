...
public class IsTypeOfOperation extends AbstractOperation{
	...
	public Object execute(Object obj, AST ast, IEolContext context) throws EolRuntimeException {
		...
		if (type!=null){
			return new EolBoolean(type.isType(obj));
		}
		else {
		...
		if (ast.getNumberOfChildren() != 1 || 
			...
			if (context.getModelRepository().isModelElementType(type)){
				return new EolBoolean(context.getModelRepository().allOfClass(type).contains(obj));
			}
			else {
