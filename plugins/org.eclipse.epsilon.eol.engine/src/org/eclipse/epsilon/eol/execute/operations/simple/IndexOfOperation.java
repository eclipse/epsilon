...
public class IndexOfOperation extends AbstractOperation{
	...
	public Object execute(Object obj, AST ast, IEolContext context) throws EolRuntimeException {
		...
		if (!(obj instanceof EolSequence)){
			throw new EolRuntimeException("Operation indexOf applies only to objects of type Sequence", ast);
		}
		
		Object parameter = context.getExecutorFactory().executeAST(ast.getFirstChild().getFirstChild(), context);
		
		EolSequence sequence = (EolSequence) obj;
		
		return sequence.indexOf(parameter);
