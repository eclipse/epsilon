          ...
          steps {
		    ...
		    lock('download-area') {
		      sshagent (['projects-storage.eclipse.org-bot-ssh']) {
			    sh '''
				  ssh genie.epsilon@projects-storage.eclipse.org 'cd /home/data/httpd/download.eclipse.org/epsilon/interim && declare -a folders=("features" "plugins"); for folder in "${folders[@]}"; do cd $folder && for jar in $(ls ${folder}/*.jar | xargs -n 1 basename); do echo "Signing ${jar}..." && curl --create-dirs -o "signed/$jar" -F "file=@$jar" http://build.eclipse.org:31338/sign; done && mv signed/* . && rm -rf signed && cd ../; done;'
				'''
		      }
		    }
          }
