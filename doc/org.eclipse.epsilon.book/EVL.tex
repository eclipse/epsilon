...
Since 2.0, \emph{constraint}s and/or \emph{context}s can be annotated as \emph{parallel} to execute each applicable element in parallel to others. This is only applicable when using the annotation-based parallel execution engine, otherwise parallelisation is applied automatically by the other parallel execution implementations.
