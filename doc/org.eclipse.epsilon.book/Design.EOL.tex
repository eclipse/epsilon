...
Also, EOL collections support the following first-order operations. If operating on large collections with computationally expensive expressions, each of the following first-order operations also has a parallel variant which may improve performance. An operation can be invoked with a parallel execution algorithm by pre-pending it with \emph{parallel} and capitalising the first letter. For example, \emph{select} becomes \emph{parallelSelect}.
