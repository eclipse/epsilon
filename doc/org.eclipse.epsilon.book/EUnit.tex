...
\begin{lstlisting}[float=tbp, language=XML,columns=fixed,caption=Format of an invocation of the EUnit Ant task,label=lst:example-eunit-ant]
<epsilon.eunit src="..."
    [failOnErrors="..."]
    [package=".."]
    [toDir="..."]
...
\begin{lstlisting}[float=tbp, language=EOL,caption=Example of a 2-level data binding,label=lst:eunit-databind,columns=fixed]
@data x
...
\begin{lstlisting}[float=tbp, language=EOL,caption=Example of reusing the same operation for several data bindings,label=lst:eunit-databind-reused,columns=fixed]
@data x
@data y
...
\begin{lstlisting}[float=tbp, language=EOL, caption=EOL script using \eol{runTarget} to run ETL, label=lst:eunit-ex1-eol]
@test
...
\begin{lstlisting}[float=tbp, language=XML,caption=Ant buildfile which only runs the EOL script,label=lst:ex2-ant]
<project>
  <target name="run-tests">
...
\begin{lstlisting}[float=tbp, language=EOL,caption=EOL script with inlined models and tasks,label=lst:ex2-eol]
@model
...
\begin{lstlisting}[float=tbp, language=XML,caption=Testing an ATL model transformation with EUnit,label=lst:eunit-atl]
<project>
  <!-- ... omitted ... -->
  <target name="atl">
...
\begin{lstlisting}[float=tbp, language=EOL,caption=Testing an EVL model validation with EUnit,label=lst:eunit-evl]
@test
operation valid() {
