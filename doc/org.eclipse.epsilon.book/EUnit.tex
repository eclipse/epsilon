...
The Ant buildfile is shown in Listing~\ref{lst:eunit-ex1-ant}. It has two targets: \anttarget{run-tests} (lines 2--22) registers the metamodels in advance\footnote{Using the \xmlattribute{metamodelfile} attribute in \xmlelement{epsilon.emf.loadModel} is discouraged, as it will result in models that belong to different \class{EPackage} instances and therefore cannot be reliably compared.} and invokes the EUnit suite, and \anttarget{tree2graph} (lines 23--28) is a helper target which transforms model \modelname{Tree} into model \modelname{Graph} using ETL. The \xmlelement{modelTasks} nested element is used to load the input, expected output and output EMF models. \modelname{Graph} is loaded with \xmlattribute{read} set to \eol{false}: the model will be initially empty, and will be populated by the ETL transformation.
