...
\begin{lstlisting}[float=tbp, caption=The simplest form of a wizard for refactoring a class into a singleton, label=lst:Basic, language=EWL]
do :
...
\begin{lstlisting}[float=tbp, caption=The wizard of Listing \ref{lst:Basic} enhanced with an $if$ condition, label=lst:WithoutGuard, language=EWL]
do : 
...
\begin{lstlisting}[float=tbp, caption=Concrete syntax of EWL wizards, label=lst:EwlConcreteSyntax, language=EWL]
wizard <name> {
